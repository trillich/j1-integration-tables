...
cte_adv as (
    select
        pop.ID_NUM,
        pop.DIV_CDE,
        ast1.ADVISOR_ID             prim_adv,
        acm1.AlternateContact       prim_adv_email,
        ast2.ADVISOR_ID             sec_adv,
        acm2.AlternateContact       sec_adv_email,
        ast3.ADVISOR_ID             career_adv,
        acm3.AlternateContact       career_adv_email,
        1 x
    from cte_pop pop
    join STUD_TERM_SUM_DIV stsd WITH (nolock)
        on (pop.id_num = stsd.ID_NUM
            and pop.DIV_CDE = stsd.DIV_CDE
            and stsd.YR_CDE = @curyr
            and stsd.TRM_CDE = @cterm
        )
    -- primary advisor:
    LEFT JOIN ADVISOR_STUD_TABLE ast1 WITH (nolock)
